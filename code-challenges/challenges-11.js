...
function square(arr) {
    const sequre = arr.map((num) => num*num);
    return sequre
...
function fullName(arr) {
    const Names = arr.map((obj) => `${obj.firstName} ${obj.lastName}`);
    return Names
    // write your code here
} 
// -------------------------------------------------------------------------------------------------------
...
function gradesAvg(arr) {
    const average =arr.map((obj) =>{
      let total =obj.gradsList.reduce((current,next)=>{
      return current+ next ;},0);
      let avg =total/obj.gradsList.length;
      return{...obj,avg};});
