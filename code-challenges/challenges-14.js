...
const replaceWords = (str) => {
    ...
    // write your code here
    const wordInput= str.split(' ');// to make it array and separate between words ,
    for (let i = 0; i < wordInput.length; i++) {
      if (wordInput[i] === 'I') {
        wordInput.splice(i, 1, 'We');//replace 
      } else if (wordInput[i] === 'am') {
        wordInput.splice(i, 1, 'are');//replace
      } else if (wordInput[i] === 'was') {
        wordInput.splice(i, 1, 'were');//replace
      }
    } 
    const newOutput = wordInput.join(' ');
